...
const templateMapper = (() => {
  ...
  function detectMapping(workbook, options = {}) {
    ...
    if (p1) {
      ...
      return _postProcess(ws, { ...p1, ...validation });
    }

    // Select best sheet (optionally user-selected)
    const sheetName = options.sheetName || _selectSheet(workbook);
    ...
    if (p2) {
      ...
      if (validation.valid) return _postProcess(ws, { ...p2, ...validation });
    ...
    return _postProcess(ws, { ...base, ...validation });
  }

  // Post-process mapping to avoid picking up "rotation"/"ergonomic"/summary rows as employees,
  // and to skip non-skill columns like "Shift" / KPI summary columns.
  function _postProcess(ws, mapping) {
    if (!mapping || !ws) return mapping;
    const range = XLSX.utils.decode_range(ws['!ref'] || 'A1:A1');

    // Refine firstRosterRow: find first row below header that looks like a real person name
    const startR = Math.max(mapping.headerRow + 1, 0);
    let bestR = mapping.firstRosterRow ?? startR;

    for (let r = startR; r <= Math.min(range.e.r, startR + 200); r++) {
      const cell = ws[XLSX.utils.encode_cell({ r, c: mapping.rosterCol })];
      const s = cell && cell.v !== undefined ? String(cell.v).trim() : '';
      if (!s) continue;
      if (_looksLikeEmployeeName(s)) { bestR = r; break; }
    }
    mapping.firstRosterRow = bestR;

    // Refine firstPositionCol: skip obvious non-position columns (e.g., Shift) immediately right of roster
    let c = Math.max(mapping.firstPositionCol, mapping.rosterCol + 1);
    for (; c <= Math.min(range.e.c, mapping.rosterCol + 8); c++) {
      const cell = ws[XLSX.utils.encode_cell({ r: mapping.headerRow, c })];
      const s = cell && cell.v !== undefined ? String(cell.v).trim() : '';
      if (!s) continue;
      if (!_isNonSkillHeader(s)) break;
    }
    mapping.firstPositionCol = c;

    return mapping;
  }

  function _isNonSkillHeader(s) {
    const t = String(s).trim().toLowerCase().replace(/\s+/g, ' ');
    if (!t) return true;
    if (t === 'shift' || t === 'shft') return true;
    // summary / KPI columns that should never be treated as operations
    if (t.includes('trained jobs/person')) return true;
    if (t.includes('operations known')) return true;
    if (t.includes('operators trained')) return true;
    if (t.includes('target')) return true;
    if (t.startsWith('#')) return true;
    if (_looksLikeDate(t)) return true;
    // single small integers as headers are usually notes/columns, not operations
    if (/^\d{1,2}$/.test(t)) return true;
    return false;
  }

  function _looksLikeEmployeeName(s) {
    const t = String(s).trim();
    const low = t.toLowerCase();
    if (!t) return false;
    if (t.length < 3 || t.length > 80) return false;
    if (low.includes('employee')) return false;
    if (low.includes('rotation')) return false;
    if (low.includes('ergonomic')) return false;
    if (low.includes('difficulty')) return false;
    if (low.includes('operators trained')) return false;
    if (low.includes('trained jobs/person')) return false;
    if (low.includes('operations known')) return false;
    if (_looksLikeDate(t)) return false;
    if (t.startsWith('#')) return false;
    // Prefer patterns like "Last, First" or "First Last"
    if (t.includes(',')) return true;
    if (/\s/.test(t) && /[A-Za-zÀ-ÿ]/.test(t)) return true;
    return false;
  }

  function _looksLikeDate(s) {
    const t = String(s).trim();
    // mm-dd, m-d, mm/dd, m/d, yyyy-mm-dd
  ...
  function extractData(ws, mapping) {
    ...
    // Collect position headers (operations), skipping summary/date/shift columns
    const positions = [];
    let consecutive = 0;
    for (let c = firstPositionCol; c <= range.e.c && consecutive < 4; c++) {
      const cell = ws[XLSX.utils.encode_cell({ r: headerRow, c })];
      const raw = cell && cell.v !== undefined ? clean(cell.v) : '';
      if (raw) {
        if (!_isNonSkillHeader(raw)) positions.push({ col: c, name: raw });
        consecutive = 0;
      } else consecutive++;
    }

    // Collect employees (roster), skipping non-name rows
    const employees = [];
    consecutive = 0;
    for (let r = firstRosterRow; r <= range.e.r && consecutive < 4; r++) {
      const cell = ws[XLSX.utils.encode_cell({ r, c: rosterCol })];
      const raw = cell && cell.v !== undefined ? clean(cell.v) : '';
      if (raw) {
        if (_looksLikeEmployeeName(raw)) employees.push({ row: r, name: raw });
        consecutive = 0;
      } else consecutive++;
    }
    ...
    // Parse skill levels
    const skills = [];
    const parseLevel = (cell) => {
      if (!cell) return 0;
      const v = cell.v !== undefined ? cell.v : cell.w;
      if (v === null || v === undefined) return 0;
      if (typeof v === 'number') {
        const n = Math.round(v);
        return (isNaN(n) ? 0 : Math.min(4, Math.max(0, n)));
      }
      const s = String(v).trim();
      if (!s) return 0;
      // allow "L1", "Level 2", "3", "3.0"
      const m = s.match(/([1-4])/);
      if (!m) return 0;
      const n = Number(m[1]);
