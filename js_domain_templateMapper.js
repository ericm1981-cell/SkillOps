...
const templateMapper = (() => {
  ...
  function _selectSheet(wb, options = {}) {
    ...
    // If caller provided an explicit sheet name, use it when present
    if (options.sheetName && wb.SheetNames.includes(options.sheetName)) return options.sheetName;

    // If multiple sheets and caller wants prompting, ask user once
    if (options.promptSheetSelection && wb.SheetNames.length > 1 && typeof window !== 'undefined' && window.prompt) {
      try {
        const list = wb.SheetNames.map((n,i)=>`${i+1}: ${n}`).join('\n');
        const ans = window.prompt(`Select the sheet/tab to import:\n\n${list}\n\nEnter a number (1-${wb.SheetNames.length}) or exact name:`);
        if (ans) {
          const trimmed = String(ans).trim();
          const asNum = Number(trimmed);
          if (Number.isFinite(asNum) && asNum>=1 && asNum<=wb.SheetNames.length) return wb.SheetNames[asNum-1];
...
let best = wb.SheetNames[0], bestScore = -1;
  ...
  function extractData(ws, mapping) {
    ...
    function _looksDateLike(s){
      const v=_norm(s);
      if (!v) return false;
      if (/^\d+(\.\d+)?$/.test(v)) return true;
      if (/^\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}$/.test(v)) return true;
      if (/^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}$/.test(v)) return true;
      if (/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)\b/.test(v)) return true;
      return false;
    }
    function _isSummaryHeader(s){
      const v=_norm(s);
      if (!v) return true;
      ...
      if (bad.some(b=>v.includes(b))) return true;
      if (/\blevel\s*[34]\b/.test(v)) return true;
      if (_looksDateLike(v)) return true;
      return false;
    }

    for (let c = firstPositionCol; c <= range.e.c && consecutive < 6; c++) {
      const cell = ws[XLSX.utils.encode_cell({ r: headerRow, c })];
      if (cell && cell.v !== undefined && String(cell.v).trim()) {
        const name = String(cell.v).trim();
        if (!_isSummaryHeader(name)) {
    ...
    const employees = [];
    consecutive = 0;
    for (let r = firstRosterRow; r <= range.e.r && consecutive < 6; r++) {
      const cell = ws[XLSX.utils.encode_cell({ r, c: rosterCol })];
      if (cell && cell.v && String(cell.v).trim().length >= 2) {
        const name = String(cell.v).trim();
        const n = name.toLowerCase();
        if (n.includes('rotation') || n.includes('target') || n.includes('trained jobs') || n.includes('operations known')) {
          consecutive = 0;
          continue;
        }
        if (NAME_RE.test(name) || name.length >= 3) {
...
const cell = ws[XLSX.utils.encode_cell({ r: emp.row, c: pos.col })];
        if (cell && (cell.v !== undefined || cell.w !== undefined)) {
          let raw = (cell.v !== undefined ? cell.v : cell.w);
          let s = String(raw ?? '').trim().toUpperCase();
          let level = 0;

          if (/^L\d$/.test(s)) level = Number(s.replace('L',''));
          else if (/^\d\s*\/\s*\d$/.test(s)) level = Number(s.split('/')[0].trim());
          else if (!isNaN(Number(s))) level = Number(s);

          if (!Number.isFinite(level) || level < 0 || level > 10) level = 0;
  ...
  function writeExport(workbook, mapping, exportData) {
    ...
    // Basic formatting: column widths and freeze panes (best-effort; conditional formatting not supported here)
    try {
      ws['!cols'] = ws['!cols'] || [];
      ws['!cols'][rosterCol] = { wch: 24 };
      for (let i = 0; i < positions.length; i++) ws['!cols'][firstPositionCol + i] = { wch: 14 };
      // Freeze header + roster columns (SheetJS uses workbook views; this is best-effort)
      ws['!freeze'] = { xSplit: firstPositionCol, ySplit: firstRosterRow, topLeftCell: XLSX.utils.encode_cell({ r: firstRosterRow, c: firstPositionCol }) };
    } catch(e) {}
