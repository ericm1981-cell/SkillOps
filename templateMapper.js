...
const templateMapper = (() => {
  ...
  function detectMapping(workbook, options = {}) {
    ...
    // If caller forces a specific tab/sheet, only analyze that sheet.
    const forcedSheet = options.sheetName || options.sheet || null;
    if (forcedSheet && workbook.Sheets[forcedSheet]) {
      const wsForced = workbook.Sheets[forcedSheet];

      // Phase 2: Keyword anchors
      const p2f = _tryKeywords(wsForced, forcedSheet);
      if (p2f) {
        const validation = _validate(wsForced, p2f, minPositions, minRoster, minRatio);
        if (validation.valid) return { ...p2f, ...validation };
      }

      // Phase 3: Structural
      const p3f = _tryStructural(wsForced, forcedSheet);
      const basef = p3f || p2f || { sheet: forcedSheet, headerRow: 0, rosterCol: 0, firstPositionCol: 1, firstRosterRow: 1, method: 'structural' };
      const validationf = _validate(wsForced, basef, minPositions, minRoster, minRatio);
      return { ...basef, ...validationf };
    }
