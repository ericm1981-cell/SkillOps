...
const importExportService = {
  async importExcel(file, lineId) {
    ...
    // If workbook has multiple tabs, let user choose which one to import.
    // (No UI changes needed — uses a simple prompt on first import attempt.)
    let forcedSheet = null;
    if (wb.SheetNames && wb.SheetNames.length > 1) {
      const list = wb.SheetNames.map((n, i) => `${i + 1}) ${n}`).join('\n');
      const pick = window.prompt(`This Excel file has multiple tabs.\n\nSelect the tab number to import:\n${list}\n\n(Press Cancel to abort)`, "1");
      if (pick === null) return null;
      const idx = parseInt(String(pick).trim(), 10);
      if (!isNaN(idx) && idx >= 1 && idx <= wb.SheetNames.length) forcedSheet = wb.SheetNames[idx - 1];
    }

    const detection = templateMapper.detectMapping(wb, forcedSheet ? { sheetName: forcedSheet } : {});
...
/* ─────────────────────────────────────────────────────────────────────────
   resetService — Factory reset (clears all local data on this device)
   Notes:
   - This app is currently single-device (IndexedDB/local storage). A reset only affects the current browser/device.
   - GitHub Pages cannot provide true multi-user shared storage without a backend (Firebase/Supabase/etc.).
───────────────────────────────────────────────────────────────────────── */
const resetService = {
  async factoryReset() {
    const ok = window.confirm(
      'FACTORY RESET\n\nThis will permanently delete ALL SkillOps data stored in this browser (lines, employees, skills, logs, templates, settings).\n\nContinue?'
    );
    if (!ok) return false;

    // 1) Try to clear known object stores via DB wrapper (if supported)
    const stores = [
      'lines','employees','positions','skillRecords','users','attendance',
      'rotationPlans','auditLogs','trainingLogs','pendingRecommendations',
      'devices','templateMappings','settings'
    ];

    try {
      if (typeof DB.clear === 'function') {
        for (const s of stores) {
          try { await DB.clear(s); } catch (e) { /* ignore missing store */ }
        }
      } else if (typeof DB._db === 'object' && DB._db && typeof DB._db.close === 'function') {
        try { DB._db.close(); } catch(e) {}
      }
    } catch (e) {
      // ignore and proceed to deleteDatabase fallback
    }

    // 2) Delete IndexedDB databases (best-effort; names vary by build)
    const dbNames = ['SkillOpsDB', 'skillops', 'SkillOps', 'SkillOps_DB'];
    await Promise.all(dbNames.map(name => new Promise(res => {
      try {
    ...
    // 3) Clear localStorage keys (best-effort)
    try {
      Object.keys(localStorage).forEach(k => {
        if (k.toLowerCase().includes('skillops')) localStorage.removeItem(k);
      });
    } catch (e) {}
...
// Add a small reset button that works even without UI changes.
// Shows as a floating button in the bottom-left.
(function attachFactoryResetButton(){
  try {
    if (document.getElementById('skillopsFactoryResetBtn')) return;
    const btn = document.createElement('button');
    btn.id = 'skillopsFactoryResetBtn';
    btn.type = 'button';
    ...
    btn.addEventListener('click', () => resetService.factoryReset());

    const add = () => document.body && document.body.appendChild(btn);
