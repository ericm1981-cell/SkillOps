...
const importExportService = {
  async importExcel(file, lineIdOrOpts) {
    const opts = (lineIdOrOpts && typeof lineIdOrOpts === 'object') ? lineIdOrOpts : {};
    let lineId = (lineIdOrOpts && typeof lineIdOrOpts === 'object') ? (lineIdOrOpts.lineId || null) : lineIdOrOpts;
    if (!window.XLSX) throw new Error(t('no_xlsx'));

    const buf = await _readFile(file);
    const wb = XLSX.read(buf, { type: 'array' });
    const detection = templateMapper.detectMapping(wb, { sheetName: opts.sheetName || opts.sheet, shift: opts.shift });

    if (!detection.valid) {
      lineId = await this._ensureLineId(lineId, opts, file.name, detection, /*createIfMissing*/ false);
      return { needsReview: true, detection, workbook: wb, binary: new Uint8Array(buf), lineIdSuggestion: lineId, suggestedLineName: (opts.lineName || detection.lineName || null) };
    }

    lineId = await this._ensureLineId(lineId, opts, file.name, detection);

    return await this._processImport(wb, detection, lineId, new Uint8Array(buf), file.name);
  },

  
  async _ensureLineId(lineId, opts, fileName, detection, createIfMissing = true) {
    if (lineId) return lineId;

    const cleanName = (raw) => {
      let n = (raw || '').toString().trim();
      n = n.replace(/\.(xlsx|xls)$/i, '');
      n = n.replace(/^skills\s*matrix\s*[_-]*/i, '');
      n = n.replace(/^skills\s*matrix\s*/i, '');
      n = n.replace(/\s*future\s*state\s*\d+$/i, '');
      n = n.replace(/\s*\(.*?\)\s*$/i, ''); // trailing parenthetical
      return n.trim() || 'Line';
    };

    const name = cleanName(opts.lineName || (detection && detection.lineName) || fileName);
    const shift = (opts.shift || '').toString().trim() || null;

    // Try to reuse existing line by name (case-insensitive)
    const lines = await DB.getAll('lines');
    const norm = name.toLowerCase();
    const existing = lines.find(l => (l.name || '').toString().trim().toLowerCase() === norm);
    if (existing) return existing.id;

    if (!createIfMissing) return null;

    return await lineService.addLine(name, shift);
  },
