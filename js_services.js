...
const importExportService = {
  async importExcel(file, lineId, options = {}) {
    ...
    // Allow the UI to pick a sheet; otherwise templateMapper will auto-select
    const detection = templateMapper.detect(wb, { sheetName: options.sheetName });

    if (!detection.valid) {
      throw new Error((detection.errors || ['Template mismatch']).join('; '));
    }

    const ws = wb.Sheets[detection.sheet];
    const extracted = templateMapper.extract(ws, detection);

    // Determine / create line if not provided
    let effectiveLineId = lineId;
    if (!effectiveLineId) {
      const inferred = _inferLineFromFile(file, options, detection);
      effectiveLineId = await lineService.addLine(inferred.name, inferred.shift);
    }
    ...
    for (const p of extracted.positions) {
      const pn = p.name;
      if (!pn) continue;
      const id = await DB.add('positions', { lineId: effectiveLineId, name: pn, createdAt: Date.now() });
      posMap.set(pn, id);
    }
    ...
    for (const e of extracted.employees) {
      const en = e.name;
      if (!en) continue;
      const id = await DB.add('employees', { lineId: effectiveLineId, name: en, role: 'Operator', status: 'Active', createdAt: Date.now() });
      employeeMap.set(en, id);
    }

    // Add skill records
    for (const s of extracted.skills) {
      const eid = employeeMap.get(s.empName);
      const pid = posMap.get(s.posName);
      if (!eid || !pid) continue;
      const level = Number(s.level) || 0;
      if (!level) continue;
      await DB.add('skillRecords', { lineId: effectiveLineId, employeeId: eid, positionId: pid, level, updatedAt: Date.now() });
    }

    await lineService.loadLineData(effectiveLineId);
    return { lineId: effectiveLineId, sheet: detection.sheet, imported: { employees: extracted.employees.length, positions: extracted.positions.length, skills: extracted.skills.length } };
  },

  // Deletes ALL local data (factory reset)
  async factoryReset() {
    try {
      ...
      if (indexedDB && indexedDB.databases) {
        ...
        await new Promise(res => {
          const req = indexedDB.deleteDatabase('SkillOpsDB');
          req.onsuccess = req.onerror = req.onblocked = () => res();
        });
      }
    } finally {
