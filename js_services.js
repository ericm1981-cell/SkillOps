...
const importExportService = {
  async importExcel(file, lineId, options = {}) {
    ...
    const detection = templateMapper.detectMapping(wb, { sheetName: options.sheetName, shift: options.shift });

    if (!detection.valid) {
      return { needsReview: true, detection, workbook: wb, binary: new Uint8Array(buf) };
    }

    // If no line selected, infer and auto-create one
    if (!lineId) {
      const inferredName = (options.lineName || _inferLineNameFromFile(file.name)) || 'Imported Line';
      const inferredShift = options.shift || null;
      lineId = await lineService.addLine(inferredName, inferredShift);
    }
