...
const lineService = {
  ...
  async addPosition(lineId, name, critical, sortOrder) {
    const norm = name.trim().toLowerCase();
    const data = { lineId, name: name.trim(), normalizedName: norm, critical: !!critical, sortOrder: sortOrder || 0, active: true, createdAt: Date.now() };
    let id;
    try {
...
const importExportService = {
  async importExcel(file, lineId, options = {
    // Auto-create line if not provided
    const _inferLineName = (fn) => {
      const base = String(fn || '').replace(/\.[^.]+$/, '');
      const m = base.match(/(?:^|\b)(line\s*\d+|line\d+|casement|mulling|accessories|pictures\s*x\s*sliders|px\s*\/\s*sl|doors)\b/i);
      if (m) return m[1].replace(/\s+/g,' ').trim();
      // Fallback to filename base
      return base.trim() || 'Line';
    };
    const _inferShift = (sheetName) => {
    ...
    if (!lineId) {
      const lineName = options.lineName || _inferLineName(file?.name);
      const shift = options.shift || null;
      try {
        lineId = await lineService.addLine(lineName, shift);
      } catch (e) {
        // If addLine failed, continue (import will likely fail with better error)
      }
    }

    // If workbook has multiple tabs and caller didn't provide sheetName, prompt the user
}) {
    if (!window.XLSX) throw new Error(t('no_xlsx'));

    const buf = await _readFile(file);
    const wb = XLSX.read(buf, { type: 'array' });
    const detection = templateMapper.detectMapping(wb, { sheetName: options.sheetName || null, promptSheetSelection: true, shift: options.shift || null });

    // If shift not provided, try to infer from selected sheet name
    if (lineId && (!options.shift || options.shift === '')) {
      const inferred = _inferShift(detection.sheet);
      if (inferred) {
        try {
          const line = await DB.get('lines', lineId);
          if (line && !line.shift) await DB.put('lines', { ...line, shift: inferred });
        } catch(e) {}
      }
    }
...
// ---- SkillOps Admin Utilities ----
(function(){
  async function factoryReset() {
    const ok = (typeof window !== 'undefined') && window.confirm
      ? window.confirm('FACTORY RESET: This will delete all SkillOps data on this device/browser. Continue?')
      : true;
    if (!ok) return;

    try { localStorage.clear(); } catch(e) {}
    // Clear common IndexedDB databases (best effort)
    if (typeof indexedDB !== 'undefined' && indexedDB.databases) {
      ...
      // Fallback: known DB names used by many demos
      ['skillops','SkillOps','SkillOpsDB','db','app'].forEach(n => { try { indexedDB.deleteDatabase(n); } catch(e) {} });
    }

    try { location.reload(); } catch(e) {}
  }

  function installFactoryResetButton() {
    ...
    // Only show when admin/authority is likely enabled
    const show = () => {
      const role = (()=>{ try { return localStorage.getItem('skillops_deviceRole') || localStorage.getItem('deviceRole'); } catch(e){ return null; }})();
      const isAdmin = /admin=1/i.test(location.search) || role === 'authority' || role === 'admin';
  ...
  if (typeof window !== 'undefined') {
    window.SkillOps = window.SkillOps || {};
    window.SkillOps.factoryReset = factoryReset;
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installFactoryResetButton);
    else installFactoryResetButton();
  }
})();
